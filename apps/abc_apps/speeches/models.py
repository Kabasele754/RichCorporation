# apps/abc_apps/speeches/models.py
...
class Speech(TimeStampedModel):
    ...
    # option speech mensuel (si tu veux aussi stocker, sinon period.key suffit)
    month = models.CharField(max_length=7, validators=[month_validator], null=True, blank=True)

    # ✅ NEW
    category = models.CharField(max_length=20, choices=CATEGORY, default="info")
...
class SpeechCoaching(TimeStampedModel):
    ...
    word_tips = models.JSONField(default=list, blank=True)
    is_final = models.BooleanField(default=False)
...
class SpeechAudio(TimeStampedModel):
    ...
    class Meta:
        indexes = [models.Index(fields=["speech", "kind", "created_at"])]
